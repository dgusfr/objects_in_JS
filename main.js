...
//Utilização do localStorage para armazenar os itens da lista
function atualizaLocalStorage() {
  localStorage.setItem("listaDeItens", JSON.stringify(listaDeItens));
}

if (listaRecuperada) {
  //Transforma o item adicionado de JSON para Objeto JavaScript
...
//Evento de escuta para quando o botão de "Salvar Item" for clicado
form.addEventListener("submit", function (evento) {
  //comportamento padrão =>  recarregar página
...
function mostrarItem() {
  ...
  deletarObjetos.forEach((i) => {
    i.addEventListener("click", (evento) => {
      valorDoElemento =
        evento.target.parentElement.parentElement.getAttribute("data-value");
      listaDeItens.splice(indiceParaRemover, 1);
      mostrarItem();
    });
  });
